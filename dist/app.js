...
/* ---------------------------------------------------------------- header */
function header() {
  const nav = [['translations', 'translate', 'Переводы'], ['progress', 'chart', 'Прогресс'], ['admin', 'shield', 'Администрирование'], ['guide', 'book', 'Руководство']];
  const langs = db.languages.map(l => opt(l.code, `${l.name} (${l.native})`, S.lang === l.code)).join('');
  return `<header class="header">
    ...
    ${dd('version-select', 'Версия', ['17.0', '18.0'].map(v => opt(v, 'ENCY ' + v, S.version === v)).join(''))}
    ${dd('language-select', 'Язык', langs)}
    <span class="spacer"></span>
    <button class="btn btn--ghost" data-act="profile" title="Роль в прототипе">${avatar(ME)}<span>${isAdmin() ? 'Администратор' : 'Переводчик · ' + esc(db.dealers.find(d => d.id === S.dealer)?.name || '')}</span>${icon('down')}</button>
    <button class="ibtn ibtn--24" data-act="theme" title="Светлая / тёмная тема">${icon(db.theme === 'dark' ? 'sun' : 'moon')}</button>
    <button class="btn btn--primary" data-act="export">${icon('download')}<span>Скачать .lng</span></button>
  </header>`;
}
...
function appsTree() {
  const all = getStats(rows.filter(r => !S.mine || r.owner === ME));
  return `<aside class="panel panel--tree"><div class="panel__head"><span class="panel__title">Приложения<span class="panel__count">${db.apps.length}</span></span><span class="spacer"></span>
    <label class="tgl" title="Только строки, за которые отвечаю я"><input type="checkbox" id="mine-toggle" ${S.mine ? 'checked' : ''}><span class="tgl__track"></span><span>Мои</span></label></div>
    <div class="panel__body tree">
      <button class="trow trow--all ${S.app === 'all' ? 'is-selected' : ''}" data-app="all">${icon('layers')}<span class="trow__name">Все приложения</span><span class="pct">${all.percent}%</span></button>
      ${GROUPS.map(g => {
        const apps = db.apps.filter(a => a.group === g);
        if (!apps.length) return '';
        const open = S.openGroups.has(g) || apps.some(a => a.id === S.app);
        const gst = getStats(rows.filter(r => apps.some(a => a.id === r.app) && (!S.mine || r.owner === ME)));
        ...
        ${open ? apps.map(a => { const st = getStats(rows.filter(r => r.app === a.id && (!S.mine || r.owner === ME))); return `<button class="trow ${S.app === a.id ? 'is-selected' : ''}" data-app="${esc(a.id)}" title="${esc(a.id)} · ${esc(a.description)}">
          <span class="trow__name">${esc(a.name)}</span>${bar(st)}<span class="pct">${st.percent}%</span></button>`; }).join('') : ''}`;
      }).join('')}
    </div></aside>`;
}
...
function stringsPanel() {
  ...
  const chips = [['all', 'Все', st.total, ''], ['untranslated', 'Без перевода', st.untranslated, '--st-untranslated'], ['auto', 'Автоперевод', st.auto, '--st-auto'], ['outdated', 'Устарело', st.outdated, '--st-outdated'], ['translated', 'Переведено', st.translated, '--st-translated'], ['errors', 'С ошибками', errors, '--st-error'], ['ignored', 'Ignored', st.ignored, '']];
  return `<section class="panel panel--strings">
    <div class="panel__head"><span class="panel__title">${esc(app ? app.name : 'Все приложения')}${app ? `<span class="panel__count">${esc(app.id)}</span>` : ''}</span>
      <span class="tag">${esc(currentLanguage().tag)} · ENCY ${esc(S.version)}</span><span class="spacer"></span>
      <span class="pct" title="Доля Translated среди строк без Ignored">${st.percent}% · ${fmt(st.translated)} из ${fmt(st.total)}</span>
      <button class="btn" data-act="autotranslate" ${!canEdit() ? 'disabled' : ''} title="Предложить автоперевод для строк без перевода">${icon('sparkles')}<span>Автоперевод</span></button></div>
    <div class="toolbar">
      <label class="field field--search">${icon('search')}<input id="string-search" type="search" value="${esc(S.query)}" placeholder="Ключ, текст, перевод, ответственный" autocomplete="off"><kbd>/</kbd></label>
      ${dd('module-select', 'Модуль', [opt('all', 'Все модули', S.module === 'all'), ...Object.entries(MODULES).map(([id, m]) => opt(id, m.label, S.module === id))].join(''))}
      ...
      ${S.status !== 'all' || S.module !== 'all' || S.query || S.mine ? `<button class="ibtn" data-act="reset-filters" title="Сбросить фильтры">${icon('close')}</button>` : ''}
    </div>
    <div class="panel__body" id="strings-list">
      <div class="grid-head"><span>English <small>эталон</small></span><span class="col-tr">${esc(currentLanguage().name)}</span><span class="col-meta">Модуль</span><span class="col-owner">Ответственный</span><span>Статус</span></div>
    ...
    <div class="panel__foot"><span>${fmt(list.length)} строк${st.ignored ? ` · ${st.ignored} ignored` : ''}</span><span class="spacer"></span><span><kbd>J</kbd>/<kbd>K</kbd> строки · <kbd>Ctrl ↵</kbd> применить</span></div>
  </section>`;
}
...
function stringRow(r) {
  ...
  return `<button class="srow ${S.selected === r.id ? 'is-selected' : ''} ${ig ? 'is-ignored' : ''}" data-row="${esc(r.id)}" title="${esc(r.id)}">
    ...
    <span class="srow__meta"><span class="ellipsis">${esc(MODULES[r.module]?.label || r.module)}</span></span>
    <span class="srow__owner ${r.owner === ME ? 'is-me' : ''}">${avatar(r.owner)}<span class="ellipsis">${esc(r.owner)}</span></span>
...
function inspector() {
  ...
  if (!row) return `<aside class="panel panel--insp"><div class="panel__head"><span class="panel__title">Строка</span></div><div class="empty-state">${icon('translate')}<b>Выберите строку</b><span>Слева — список, здесь — перевод и контекст</span></div></aside>`;
  ...
  return `<aside class="panel panel--insp">
    <div class="panel__head"><span class="panel__title">Строка</span><span class="spacer"></span>
    ...
    <div class="insp"><div class="insp__scroll">
      <div class="trow-title"><span class="ellipsis" title="${esc(row.code)}">${esc(row.code)}</span>${ig ? statusBadge('ignored', 'Ignored') : statusBadge(t.status)}</div>
      ...
      <div class="irow"><span class="irow__label">Приложение</span><span class="irow__val"><span class="ellipsis">${esc(app?.name || row.app)} <span class="muted">${esc(row.app)}</span></span></span></div>
      <div class="irow"><span class="irow__label">Модуль</span><span class="irow__val"><span class="tag">${esc(MODULES[row.module]?.label || row.module)}</span><span class="muted ellipsis">${esc(row.context)}</span></span></div>
      ...
      <div class="irow"><span class="irow__label">Ignored</span><span class="irow__val"><label class="tgl" title="Строку переводить не нужно: технический идентификатор, ось, формула"><input type="checkbox" id="ignore-toggle" ${ig ? 'checked' : ''} ${!isAdmin() || db.excludedModules.includes(row.module) ? 'disabled' : ''}><span class="tgl__track"></span><span>${db.excludedModules.includes(row.module) ? 'модуль исключён' : ig ? 'не переводить' : 'переводить'}</span></label>${row.reason && ig ? `<span class="muted">· ${esc(row.reason)}</span>` : ''}</span></div>

      <div class="sect"><div class="sect__head"><span class="lang">EN</span><b>Эталон</b><button class="ibtn" data-act="copy-source" title="Скопировать">${icon('copy')}</button></div>
        <div class="srcbox">${showNl(source(row))}</div>
        ${t.status === 'outdated' && row.previousSource && S.version === '18.0' ? `<div class="notice notice--amber">${icon('history')}<span>Эталон изменился в 18.0. Было: <del>${esc(row.previousSource)}</del>. Проверьте перевод и подтвердите.</span></div>` : ''}
        ${row.note ? `<div class="notice">${icon('info')}<span>${esc(row.note)}</span></div>` : ''}</div>

      <div class="sect"><div class="sect__head"><span class="lang is-target">${esc(currentLanguage().tag)}</span><b>${esc(currentLanguage().name)}</b>${dirty ? '<span class="tag tag--amber">черновик</span>' : ''}</div>
        <div class="editor"><textarea id="translation-input" rows="3" lang="${esc(S.lang)}" spellcheck="true" placeholder="${edit ? 'Введите перевод…' : '—'}" ${edit ? '' : 'disabled'}>${esc(text)}</textarea>
          <div class="editor__tools">
            <button class="ibtn" data-act="insert-newline" title="Вставить перенос строки (Shift+Enter)" ${edit ? '' : 'disabled'}>${icon('enter')}</button>
            <button class="ibtn" data-act="suggest" title="Подставить автоперевод" ${edit ? '' : 'disabled'}>${icon('sparkles')}</button>
            <button class="ibtn" data-act="copy-source-to" title="Скопировать эталон в перевод" ${edit ? '' : 'disabled'}>${icon('arrow')}</button>
            <span class="spacer"></span><small id="char-count">${text.length} симв.</small></div></div>
        ${t.status === 'auto' && !dirty ? `<div class="notice notice--blue">${icon('sparkles')}<span>Автоперевод. Проверьте смысл и подтвердите — иначе строка не попадёт в .lng.</span></div>` : ''}
        ${!canEdit() ? `<div class="notice">${icon('lock')}<span>Язык доступен вашей компании только для чтения. Доступ выдаёт администратор.</span></div>` : ''}
        <div class="checks" id="validation-checks">${renderChecks(checks)}</div></div>

      <div class="sect"><div class="seg" role="tablist">${[['context', 'Контекст'], ['matches', 'Совпадения', matches.length], ['history', 'История', t.history.length]].map(([id, label, n]) => `<button role="tab" class="${S.tab === id ? 'is-active' : ''}" data-tab="${id}">${label}${n !== undefined ? `<small>${n}</small>` : ''}</button>`).join('')}</div>
        ${inspectorTab(row, t, matches)}</div>
    </div>
    <div class="insp__foot">${ig
      ? `<button class="btn btn--wide" data-act="ignore" ${!isAdmin() || db.excludedModules.includes(row.module) ? 'disabled' : ''}>Вернуть в перевод</button>`
      : `<div class="row"><button class="btn btn--primary" id="apply-button" data-act="apply" ${!edit || checks.some(c => !c.ok) ? 'disabled' : ''}>${icon('check')}Применить<kbd>Ctrl ↵</kbd></button><button class="btn btn--secondary" data-act="apply-next" ${!edit || checks.some(c => !c.ok) ? 'disabled' : ''}>Применить и далее</button></div>
         ${matches.length ? `<button class="btn btn--wide" data-act="apply-matches" ${!edit || !text.trim() ? 'disabled' : ''}>${icon('copy')}Перевести так же ещё ${matches.length} ${plural(matches.length, 'строку', 'строки', 'строк')}</button>` : ''}`}
    </div></div></aside>`;
}
const plural = (n, a, b, c) => { const m = n % 10, h = n % 100; return m === 1 && h !== 11 ? a : m >= 2 && m <= 4 && (h < 10 || h >= 20) ? b : c; };
const renderChecks = checks => checks.map(c => `<div class="check ${c.ok ? 'ok' : 'fail'}">${icon(c.ok ? 'check' : 'warning')}<span>${esc(c.label)}</span></div>`).join('');

function inspectorTab(row, t, matches) {
  if (S.tab === 'history') {
    if (!t.history.length) return `<p class="muted">Перевод ещё не применялся.</p>`;
    return `<div class="hist">${[...t.history].reverse().map(h => `<div class="hist__e">${avatar(h.author)}<div><div class="hist__meta"><b>${esc(h.author)}</b><span>${new Date(h.date).toLocaleString('ru-RU', {day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'})}</span>${statusBadge(h.status)}</div><div class="hist__text">${esc(h.text)}</div></div></div>`).join('')}</div>`;
  }
  if (S.tab === 'matches') {
    if (!matches.length) return `<p class="muted">Такой же исходной строки в других местах нет.</p>`;
    return `<p class="muted">Одинаковый эталон в ${matches.length} ${plural(matches.length, 'строке', 'строках', 'строках')}. Кнопка внизу применит текущий перевод ко всем отмеченным.</p>
      ${matches.map(r => { const mt = translation(r); return `<label class="match"><input type="checkbox" class="ck" name="match-id" value="${esc(r.id)}" ${accepted(mt.status) ? '' : 'checked'}><span class="ellipsis" title="${esc(r.id)}">${esc(appById(r.app)?.name || r.app)} · ${esc(r.code)}</span>${statusBadge(mt.status)}</label>`; }).join('')}`;
  }
  return `<button class="win" data-act="context" title="Открыть схему окна"><div class="win__bar"><i></i><i></i><i></i><span>${esc(appById(row.app)?.name || row.app)} — ${esc(row.context.split(' → ')[0])}</span></div>
    <div class="win__body"><div class="win__row"><span>${esc(row.context.split(' → ')[1] || 'Параметр')}</span><span class="win__ctl is-hl">${esc(source(row).split('\n')[0])}</span></div><div class="win__row"><span>Значение</span><span class="win__ctl"></span></div><div class="win__row"><span>Режим</span><span class="win__ctl"></span></div><div class="win__btns"><span>OK</span><span>Cancel</span></div></div></button>
    <p class="muted">Схема окна показывает, где строка появится в интерфейсе. В производственной версии здесь снимок окна ENCY, привязанный к ключу.</p>`;
}
...
function applyCurrent(next = false) {
  ...
  toast('Перевод применён. Строка попадёт в .lng.');
}
...
function action(name) {
  ...
  switch (name) {
    ...
    case 'apply-matches': {
      ...
      const ids = $$('input[name="match-id"]:checked').map(e => e.value);
      const targets = S.tab === 'matches' && ids.length ? rows.filter(r => ids.includes(r.id)) : rows.filter(r => r.id !== row.id && !ignored(r) && source(r) === source(row) && !accepted(translation(r).status));
      const text = currentDraft(row);
      if (!targets.length) { toast('Все одинаковые строки уже подтверждены. Отметьте их на вкладке «Совпадения», чтобы перезаписать.', 'warning'); return; }
...
document.addEventListener('click', e => {
  ...
  if (d.row) { S.selected = d.row; S.editorOpen = true; render(); return; }
...
document.addEventListener('change', e => {
  ...
  if (el.id === 'mine-toggle') { S.mine = el.checked; render(); }
...
document.addEventListener('keydown', e => {
  ...
  if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && S.page === 'translations') { e.preventDefault(); applyCurrent(); return; }
